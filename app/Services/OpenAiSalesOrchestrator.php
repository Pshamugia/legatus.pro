...
class OpenAiSalesOrchestrator
{
    ...
    public function respond(Agent $agent, Conversation $conversation, string $message): array
    {
        ...
        if (($catalogContext['is_human_request'] ?? false) === true && $agent->humanHandoffEnabled()) {
            ...
            $this->forceHandoff(
                $conversation,
                $reason,
                'The customer asked to speak with another person. Continue from the preserved conversation context.',
            );
            ...
            AgentRun::create([
                ...
                'conversation_id' => $conversation->id,
                'model' => $primaryModel,
                'route' => 'semantic_human_request',
                'status' => 'completed',
                'tools_used' => [['name' => 'resolve_human_request']],
            ...
            return $this->handoffReply($this->safeHandoffText($message), $reason, ['resolve_human_request']);
        }
        ...
        if (is_array($catalogContext) && ($catalogContext['is_catalog_follow_up'] ?? false) === true) {
            ...
            if ($resolvedQuery !== '') {
                ...
                if ($matchScope === 'exact_identity'
                    ...
                    // A syntactically valid exact lookup may still be a bad
                    // interpretation of the conversation (for example, a
                    // failed bundle search followed by a request for the same
                    // creator's individual items). Ask the stronger model to
                    // re-resolve meaning before accepting a broad "none exist"
                    // conclusion. Business facts still come only from tools.
                    $fallbackAttempted = true;
                    $alternateContext = $this->resolveCatalogFollowUp(
                        $agent,
                        $conversation,
                        $message,
                        $this->fallbackModel(),
                    ...
                    $alternateContext = $this->mergeActiveCatalogScope($conversation, $alternateContext);
                    $alternateQuery = trim((string) ($alternateContext['resolved_query'] ?? ''));
                    $alternateScope = (string) ($alternateContext['catalog_match_scope'] ?? 'exact_identity');
                    if (($alternateContext['is_catalog_follow_up'] ?? false) === true
                        && $alternateQuery !== ''
                        && ($alternateScope === 'entity_family' || $alternateQuery !== $resolvedQuery)) {
                        ...
                        $used[] = ['name' => 'semantic_resolution_fallback', 'arguments' => [], 'result' => [
                            'ok' => true,
                            'model' => $this->fallbackModel(),
                            'reason' => 'empty_exact_lookup',
                        ]];
                    }
                }
        ...
        if (in_array($scopeAction, ['continue', 'refine'], true)) {
            $budgetConstraint ??= is_numeric(data_get($conversation->context, 'active_catalog_scope.budget'))
                ? (float) data_get($conversation->context, 'active_catalog_scope.budget')
                : null;
            $quantityConstraint ??= is_numeric(data_get($conversation->context, 'active_catalog_scope.quantity'))
                ? (int) data_get($conversation->context, 'active_catalog_scope.quantity')
                : null;
        }
        ...
        for ($round = 0; $round < config('services.openai.max_tool_rounds'); $round++) {
            ...
            foreach ($calls as $call) {
                ...
                if ($call['name'] === 'recommend_products') {
                    ...
                    if (in_array($scopeAction, ['continue', 'refine'], true)) {
                        $args['exclude_product_ids'] = $this->activeCatalogProductIds($conversation)->all();
                        if (blank($args['mood'] ?? null) && filled(data_get($conversation->context, 'active_catalog_scope.mood'))) {
                            $args['mood'] = data_get($conversation->context, 'active_catalog_scope.mood');
                        }
                    }
        ...
        $this->rememberActiveCatalogScope($conversation, collect($used), $catalogContext);
    ...
    private function resolveCatalogFollowUp(
        ...
        $recentlyMentionedIds = $this->recentlyMentionedCatalogProductIds($agent, $conversation, $recentIds);
        $records = $recentIds->isEmpty() ? [] : $agent->customerProducts()->whereIn('id', $recentIds)->get()
            ->map(fn ($product): array => [
                ...
                'explicitly_mentioned_in_recent_assistant_reply' => $recentlyMentionedIds->contains((int) $product->id),
            ])->values()->all();
        ...
        try {
            ...
            $referencedProductId = (int) ($resolved['referenced_product_id'] ?? 0);
            $referencedProduct = $referencedProductId > 0 && $recentIds->contains($referencedProductId)
                ? collect($records)->firstWhere('product_id', $referencedProductId)
                : null;
            $resolved['referenced_product_id'] = is_array($referencedProduct) ? $referencedProductId : null;
            if (($resolved['is_catalog_follow_up'] ?? false) === true && is_array($referencedProduct)) {
                $resolved['catalog_scope_action'] = 'replace';
                $resolved['recommendation_scope'] = 'none';
                $resolved['recommendation_query'] = null;
                $resolved['recommendation_category'] = null;
                $resolved['recommendation_occasion'] = null;
                $resolved['resolved_query'] = (string) $referencedProduct['name'];
                $resolved['resolved_queries'] = [(string) $referencedProduct['name']];
                $resolved['resolved_category'] = null;
                $resolved['catalog_match_scope'] = 'exact_identity';
                $resolved['exclude_product_ids'] = [];
                $resolved['expects_complete_set'] = false;
            }
    ...
    private function catalogFollowUpFormat(): array
    {
        return ['type' => 'json_schema', 'name' => 'catalog_follow_up', 'strict' => true, 'schema' => [
            ...
            'properties' => [
                ...
                'referenced_product_id' => ['type' => ['integer', 'null']],
                'resolved_category' => [
                    'type' => ['string', 'null'],
            ...
            'required' => ['is_delivery_request', 'delivery_request_type', 'is_human_request', 'is_business_knowledge_request', 'knowledge_query', 'knowledge_scope', 'is_catalog_follow_up', 'catalog_scope_action', 'recommendation_scope', 'recommendation_query', 'recommendation_category', 'recommendation_occasion', 'resolved_query', 'resolved_queries', 'referenced_product_id', 'resolved_category', 'catalog_match_scope', 'exclude_product_ids', 'expects_complete_set'],
            'additionalProperties' => false,
        ]];
    }

    private function mergeActiveCatalogScope(Conversation $conversation, ?array $resolved): ?array
    {
        if (! is_array($resolved)) {
            return null;
        }

        $scope = data_get($conversation->context, 'active_catalog_scope');
        if (! is_array($scope)) {
            return $resolved;
        }

        $action = (string) ($resolved['catalog_scope_action'] ?? 'replace');
        if ($action === 'continue') {
            if (($scope['tool'] ?? null) === 'recommend_products') {
                $resolved['is_catalog_follow_up'] = false;
                $resolved['recommendation_scope'] = filled($scope['query'] ?? null) || filled($scope['category'] ?? null)
                    ? 'constrained'
                    : 'broad';
                $resolved['recommendation_query'] = filled($scope['query'] ?? null) ? $scope['query'] : null;
                $resolved['recommendation_category'] = $scope['category'] ?? null;
                $resolved['recommendation_occasion'] = $scope['occasion'] ?? null;
                $resolved['resolved_query'] = null;
                $resolved['exclude_product_ids'] = $this->activeCatalogProductIds($conversation)->all();

                return $resolved;
            }

            $query = trim((string) ($scope['query'] ?? $scope['category'] ?? ''));
            if ($query === '') {
                return $resolved;
            }

            $resolved['is_catalog_follow_up'] = true;
            $resolved['recommendation_scope'] = 'none';
            $resolved['recommendation_query'] = null;
            $resolved['recommendation_category'] = null;
            $resolved['recommendation_occasion'] = null;
        ...
        if (($scope['tool'] ?? null) === 'recommend_products'
            && ($resolved['recommendation_scope'] ?? 'none') === 'none'
            && ! ($resolved['is_catalog_follow_up'] ?? false)) {
            $resolved['recommendation_scope'] = filled($scope['query'] ?? null) || filled($scope['category'] ?? null)
                ? 'constrained'
                : 'broad';
        ...
        $resolved['recommendation_query'] = filled($resolved['recommendation_query'] ?? null)
            ? $resolved['recommendation_query']
            : ($scope['query'] ?? null);
        $resolved['recommendation_category'] = filled($resolved['recommendation_category'] ?? null)
            ? $resolved['recommendation_category']
            : ($scope['category'] ?? null);
        $resolved['recommendation_occasion'] = filled($resolved['recommendation_occasion'] ?? null)
            ? $resolved['recommendation_occasion']
            : ($scope['occasion'] ?? null);
    ...
    private function activeCatalogProductIds(Conversation $conversation): Collection
    {
        return collect(array_merge(
            (array) data_get($conversation->context, 'active_catalog_scope.shown_product_ids', []),
            $this->recentCatalogProductIds($conversation)->all(),
        ))->map(fn ($id): int => (int) $id)->filter()->unique()->values();
    }

    private function rememberActiveCatalogScope(Conversation $conversation, Collection $used, ?array $resolved): void
    {
        $resolvedQueries = collect(array_merge(
            [(string) ($resolved['resolved_query'] ?? '')],
            (array) ($resolved['resolved_queries'] ?? []),
        ))->map(fn ($query): string => mb_strtolower(trim((string) $query)))->filter()->unique();
        $call = null;
        if (($resolved['is_catalog_follow_up'] ?? false) === true
            && ($resolved['catalog_match_scope'] ?? null) === 'exact_identity') {
            $call = $used->first(function (array $call) use ($resolvedQueries): bool {
                return ($call['name'] ?? null) === 'search_products'
                    && data_get($call, 'result.ok') === true
                    && data_get($call, 'arguments._identity_match') === true
                    && $resolvedQueries->contains(mb_strtolower(trim((string) data_get($call, 'arguments.query', ''))));
            });
        }
        $call ??= $used->reverse()->first(function (array $call): bool {
            return in_array($call['name'] ?? null, ['search_products', 'recommend_products'], true)
                && data_get($call, 'result.ok') === true;
        });
        if (! is_array($call)) {
            return;
        ...
        $category = filled($arguments['category'] ?? null) ? trim((string) $arguments['category']) : null;
        if ($query === '' && $category === null) {
            return;
        }

        $context = is_array($conversation->context) ? $conversation->context : [];
        $previousShown = (array) data_get($context, 'active_catalog_scope.shown_product_ids', []);
        $context['active_catalog_scope'] = array_filter([
            'tool' => (string) $call['name'],
            'query' => $query,
            'category' => $category,
            'budget' => is_numeric($arguments['budget'] ?? null) ? (float) $arguments['budget'] : null,
            'quantity' => is_numeric($arguments['quantity'] ?? null) ? (int) $arguments['quantity'] : null,
            'max_price' => is_numeric($arguments['max_price'] ?? null) ? (float) $arguments['max_price'] : null,
            'mood' => filled($arguments['mood'] ?? null) ? (string) $arguments['mood'] : null,
            'occasion' => filled($arguments['occasion'] ?? null) ? (string) $arguments['occasion'] : null,
            'catalog_match_scope' => $resolved['catalog_match_scope'] ?? null,
            'shown_product_ids' => collect($previousShown)->map(fn ($id): int => (int) $id)->filter()->unique()->values()->all(),
    ...
    private function recentlyMentionedCatalogProductIds(Agent $agent, Conversation $conversation, Collection $recentIds): Collection
    {
        if ($recentIds->isEmpty()) {
            return collect();
        }
        ...
        foreach ($conversation->messages()->where('role', 'assistant')->latest('id')->limit(8)->get() as $message) {
            $content = mb_strtolower(Str::squish((string) $message->content));
            $mentioned = $products->filter(function ($product) use ($content): bool {
                $name = mb_strtolower(Str::squish((string) $product->name));

                return mb_strlen($name) >= 3 && str_contains($content, $name);
    ...
    private function guardrailReason(Agent $agent, Conversation $conversation, array $data, Collection $used): ?string
    {
        ...
        $hasExactIdentityResolution = $successful
            ->where('name', 'resolve_catalog_context')
            ->contains(fn (array $call): bool => data_get($call, 'result.catalog_match_scope') === 'exact_identity');
        $exactIdentityIds = $successful
            ->where('name', 'search_products')
            ->filter(fn (array $call): bool => data_get($call, 'arguments._identity_match') === true)
            ->flatMap(fn (array $call) => array_merge(
                data_get($call, 'result.products', []),
                data_get($call, 'result.unavailable_products', []),
            ))
            ->pluck('id')->map(fn ($id): int => (int) $id)->filter()->unique();
        if ($hasExactIdentityResolution
            && $exactIdentityIds->isNotEmpty()
            && $claimedProductIds->diff($exactIdentityIds)->isNotEmpty()) {
            return 'An exact product request included products outside the verified exact-identity search result.';
        }
    ...
    /**
     * Reject promises that exceed the actions exposed by the server tool
    ...
    private function unsupportedActionReason(Agent $agent, string $text): ?string
    {
        ...
        $orderData = preg_match('/(?:recipient|full\s+name|phone|telephone|delivery\s+address|shipping\s+address|card\s+(?:number|details)|payment\s+details|მიმღებ\p{L}*|სახელ(?:ი|სა\s+და\s+გვარი)|ტელეფონ\p{L}*|ნომერ\p{L}*|მისამართ\p{L}*|ბარათ\p{L}*|გადახდ\p{L}*)/iu', $text) === 1;
        $claimsOrderExecution = preg_match('/(?:\b(?:i|we)\s+(?:can|will|shall)\s+(?:place|complete|finali[sz]e|submit)\s+(?:the|your|an?)?\s*order\b|შეკვეთ\p{L}*\s+(?:გაგიფორმ\p{L}*|გავაფორმ\p{L}*|შევუკვეთ\p{L}*|განვათავს\p{L}*))/iu', $text) === 1;

        if ($claimsOrderExecution || ($orderContext && $asksCustomerToProvide && $orderData)) {
