...
<div class="chatpage" style="display:block;padding:40px 24px"><main style="max-width:1050px;margin:auto">
    ...
    @else
        ...
        @if(!$paddleClientToken || collect($plans)->contains(fn($plan) => blank($plan['price_id'])))<div class="panel" style="margin-bottom:20px;color:#9b5b12">Paddle {{ $paddleEnvironment === 'sandbox' ? 'Sandbox' : 'Live' }} configuration is incomplete. Add the client token and all three price IDs to <code>.env</code>.</div>@endif
