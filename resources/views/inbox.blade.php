...
<script nonce="{{ request()->attributes->get('csp_nonce') }}">
    ...
    const automaticReleaseUrl = @json(route('inbox.close', $selected));
    let suppressAutomaticRelease = false;
    let automaticReleaseSent = false;
    ...
    document.querySelectorAll('#conversation-workspace form').forEach((form) => {
        form.addEventListener('submit', () => { suppressAutomaticRelease = true; });
    });
    window.addEventListener('pagehide', () => {
        if (workspace.dataset.status !== 'human' || suppressAutomaticRelease || automaticReleaseSent) return;
        automaticReleaseSent = true;
        const data = new FormData();
        data.append('_token', document.querySelector('meta[name="csrf-token"]').content);
        navigator.sendBeacon(automaticReleaseUrl, data);
    });
