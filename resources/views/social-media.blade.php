...
<div class="dash-shell">
    ...
    <main class="main social-main">
        ...
        @if(session('social_success'))
            <div class="social-alert success" id="social-success-message" role="status" aria-live="polite" tabindex="-1">{{ session('social_success') }}</div>
        @endif
        @if($errors->any())
                        ...
                        <div class="template-editor">
                            ...
                            <fieldset class="image-style-picker">
                                ...
                                <div class="image-style-grid">
                                    @foreach(['three_d' => '3D catalog design', 'original' => 'Catalog design', 'storefront' => 'Storefront image', 'raw' => 'Plain photo', 'framed' => 'Classic frame', 'editorial' => 'Editorial', 'dark' => 'Dark', 'brand' => 'Legatus green'] as $style => $label)
        ...
        <section class="social-layout">
            <form class="panel schedule-form" id="schedule-form" method="post" action="{{ route('social-media.store') }}">
                @csrf
                ...
                <fieldset class="copy-mode-fieldset"><legend>Post writing</legend><p class="field-help">Keep the current catalog text, or let Luna write a channel-specific caption from the verified product title, description and image.</p>
                    ...
                    <div class="ai-tone-picker" data-ai-tones @if(old('copy_mode', 'original') !== 'ai') hidden @endif>
                        ...
                        <div class="choice-grid">
                            @foreach(['simple' => ['Simple', 'Clear, warm and concise.'], 'creative' => ['Creative', 'Vivid, memorable and playful.'], 'academic' => ['Informative', 'Precise, composed and educational.']] as $tone => [$label, $description])
                                <label class="choice-card"><input type="radio" name="ai_tone" value="{{ $tone }}" @checked(old('ai_tone', 'simple') === $tone) @disabled(!$canManage)><span><strong>{{ $label }}</strong><small>{{ $description }}</small></span></label>
                            @endforeach
                        </div>
                    </div>
                </fieldset>
                ...
                <button class="btn lime schedule-submit" type="submit" data-schedule-submit @disabled(!$canManage)>
                    <span class="schedule-submit-spinner" data-schedule-submit-spinner aria-hidden="true" hidden></span>
                    <span data-schedule-submit-label>Create automatic schedule →</span>
                </button>
                <p class="schedule-submit-status" data-schedule-submit-status role="status" aria-live="polite" hidden></p>
            </form>
        ...
        <section class="panel schedule-list">
            ...
            @forelse($schedules as $schedule)
                <article class="schedule-row">
                    <div><strong>{{ $schedule->starts_on->format('d M Y') }} — {{ $schedule->ends_on->format('d M Y') }}</strong><p>{{ $schedule->posts_per_day }} posts/day · {{ implode(', ', $schedule->providers) }} · {{ $schedule->languages ? implode(', ', $schedule->languages) : 'All languages' }} · {{ $schedule->categories ? implode(', ', $schedule->categories) : 'All categories' }}</p><small>{{ $schedule->copy_mode === 'ai' ? 'AI Copywriter · '.(['simple' => 'Simple', 'creative' => 'Creative', 'academic' => 'Informative'][$schedule->ai_tone] ?? 'Simple') : 'Original content' }}</small>@if($schedule->posting_times)<small>{{ implode(', ', $schedule->posting_times) }} · {{ $schedule->timezone }}</small>@else<small>Automatically distributed · {{ $schedule->timezone }}</small>@endif<small>{{ $schedule->published_posts_count }}/{{ $schedule->posts_count }} published @if($schedule->failed_posts_count) · {{ $schedule->failed_posts_count }} failed @endif</small>@if($schedule->story_posts_count)<small>{{ $schedule->published_stories_count }}/{{ $schedule->story_posts_count }} Meta Stories published @if($schedule->failed_stories_count) · {{ $schedule->failed_stories_count }} need attention @endif</small>@endif</div>
                    <span class="status-pill {{ $schedule->status }}">{{ ucfirst($schedule->status) }}</span>
                    @if($canManage)
                        <details class="schedule-editor">
                            <summary class="text-button">Edit</summary>
                            <form method="post" action="{{ route('social-media.update', $schedule) }}">
                                @csrf
                                @method('PUT')
                                <div class="form-grid two">
                                    <label>From date<input type="date" name="starts_on" value="{{ $schedule->starts_on->format('Y-m-d') }}" min="{{ now($schedule->timezone)->toDateString() }}" required></label>
                                    <label>To date<input type="date" name="ends_on" value="{{ $schedule->ends_on->format('Y-m-d') }}" min="{{ now($schedule->timezone)->toDateString() }}" required></label>
                                </div>
                                <div class="edit-timing-options">
                                    <label><input type="radio" name="timing_mode" value="auto" @checked(!$schedule->posting_times)> Automatically distribute</label>
                                    <label><input type="radio" name="timing_mode" value="custom" @checked($schedule->posting_times)> Choose exact times</label>
                                </div>
                                <div class="edit-posting-times" @if(!$schedule->posting_times) hidden @endif>
                                    @for($index = 0; $index < $schedule->posts_per_day; $index++)
                                        <label>Post {{ $index + 1 }}<input type="time" name="posting_times[]" value="{{ $schedule->posting_times[$index] ?? sprintf('%02d:00', $schedule->posts_per_day === 1 ? 12 : round(9 + (12 * $index / max(1, $schedule->posts_per_day - 1)))) }}"></label>
                                    @endfor
                                </div>
        ...
        <section class="panel schedule-list">
            ...
            @forelse($upcoming as $post)
                ...
                <article class="upcoming-row">
                    ...
                    <div><strong>{{ $post->title }}</strong><p>{{ ucfirst($post->provider) }}@if($post->language) · {{ $post->language }}@endif · {{ $localScheduledFor->format('d M Y, H:i') }} {{ $post->schedule?->timezone ?: 'UTC' }}</p>@if(in_array($post->provider, ['facebook', 'instagram'], true))<small>Will also publish automatically as a Story.</small>@endif @if($post->product_id)<a href="{{ $post->product_url }}" target="_blank" rel="noopener">Open public product ↗</a><details><summary>View prepared post text</summary><p class="prepared-caption">{{ $post->caption }}</p></details>@else<small>The next unused publishable product will be selected automatically at posting time.</small>@endif</div>
                    <span class="status-pill">{{ ucfirst($post->status) }}</span>
                </article>
            @empty<p class="empty-state">Scheduled posts will appear here.</p>@endforelse
        </section>

        <section class="panel schedule-list" aria-labelledby="published-facebook-heading">
            <div class="section-heading"><div><span class="step">05</span><div><h2 id="published-facebook-heading">Published Facebook Page posts</h2><p>Edit the live post text or remove a post from the connected Facebook Page.</p></div></div><span class="status-pill">pages_manage_posts</span></div>
            @forelse($publishedFacebookPosts as $post)
                <article class="published-post-row">
                    @if($post->image_url)<img src="{{ $post->image_url }}" alt="">@endif
                    <div class="published-post-content">
                        <div class="published-post-heading">
                            <div><strong>{{ $post->title }}</strong><p>Published {{ $post->published_at?->format('d M Y, H:i') ?: 'on Facebook' }}@if($post->provider_updated_at) · Edited {{ $post->provider_updated_at->format('d M Y, H:i') }}@endif</p></div>
                            <span class="status-pill {{ $post->provider_deleted_at ? 'deleted' : 'active' }}">{{ $post->provider_deleted_at ? 'Deleted from Facebook' : 'Live on Facebook' }}</span>
                        </div>
                        @if($post->provider_deleted_at)
                            <p class="prepared-caption">{{ $post->caption }}</p>
                            <small>This local history is retained for audit and duplicate-post protection.</small>
                        @elseif($canManage)
                            <a class="published-post-link" href="{{ $post->facebookPermalink() }}" target="_blank" rel="noopener">Open live post on Facebook ↗</a>
                            <form class="published-post-edit" method="post" action="{{ route('social-media.facebook-posts.update', $post) }}">
                                @csrf
                                @method('PUT')
                                <label for="facebook-caption-{{ $post->id }}"><strong>Edit Facebook post text</strong><small>This changes the text on the already-published Facebook Page post.</small></label>
                                <textarea id="facebook-caption-{{ $post->id }}" name="caption" rows="6" maxlength="5900" required>{{ $post->caption }}</textarea>
                                <button class="btn primary" type="submit">Update live Facebook post</button>
                            </form>
                            <details class="published-post-delete">
                                <summary class="text-button danger">Delete live Facebook post</summary>
                                <form method="post" action="{{ route('social-media.facebook-posts.destroy', $post) }}">
                                    @csrf
                                    @method('DELETE')
                                    <p><strong>This permanently removes the post from Facebook.</strong> The Legatus history remains available.</p>
                                    <label><input type="checkbox" name="confirm_delete" value="1" required> I understand that the Facebook post will be deleted.</label>
                                    <button class="btn danger" type="submit">Confirm Facebook post deletion</button>
                                </form>
                            </details>
                        @else
                            <p class="prepared-caption">{{ $post->caption }}</p>
            ...
            @empty<p class="empty-state">Published Facebook Page posts will appear here after Legatus publishes them.</p>@endforelse
        </section>
    </main>
</div>
...
.published-post-row{display:flex;align-items:flex-start;gap:14px;padding:18px 0;border-top:1px solid var(--line)}.published-post-row>img{width:84px;height:84px;flex:0 0 84px;border-radius:12px;object-fit:cover}.published-post-content{min-width:0;flex:1}.published-post-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:14px}.published-post-heading p{margin:5px 0;color:var(--muted);font-size:11px}.status-pill.deleted{background:#f2f2f2;color:#747474}.published-post-link{display:inline-block;margin-top:5px;color:var(--green);font-size:11px;font-weight:800}.published-post-edit{margin-top:13px;padding:15px;border:1px solid var(--line);border-radius:13px;background:#f8faf7}.published-post-edit label,.published-post-edit label small{display:block}.published-post-edit label small{margin-top:4px;color:var(--muted);font-size:10px}.published-post-edit textarea{width:100%;box-sizing:border-box;margin:10px 0;padding:12px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink);font:inherit;line-height:1.55;resize:vertical}.published-post-delete{margin-top:10px}.published-post-delete summary{cursor:pointer}.published-post-delete form{margin-top:10px;padding:15px;border:1px solid #ecc8c1;border-radius:13px;background:#fff5f2}.published-post-delete form p{margin:0 0 10px;font-size:12px}.published-post-delete form label{display:flex;align-items:center;gap:8px;font-size:11px}.published-post-delete .btn{margin-top:12px}.btn.danger{background:#a64132;color:#fff}
.timing-mode-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.timing-mode{display:flex;align-items:flex-start;gap:10px;padding:13px;border:1px solid var(--line);border-radius:12px;cursor:pointer}.timing-mode input{width:auto;margin:3px 0 0}.timing-mode span{display:flex;min-width:0;flex-direction:column}.timing-mode small{margin-top:3px;color:var(--muted);font-size:10px;line-height:1.4}.posting-times{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-top:14px;padding:14px;border:1px solid var(--line);border-radius:13px;background:#f8faf7}.posting-times[hidden]{display:none!important}.posting-time{margin:0}.posting-time input{margin-top:6px}.schedule-row small{display:block;margin-top:3px}.schedule-editor{position:relative}.schedule-editor>summary{list-style:none}.schedule-editor>summary::-webkit-details-marker{display:none}.schedule-editor form{position:absolute;z-index:4;right:0;top:28px;width:min(540px,85vw);padding:18px;border:1px solid var(--line);border-radius:14px;background:#fff;box-shadow:0 18px 50px rgba(13,47,36,.18)}.schedule-editor .form-grid{margin-bottom:12px}.edit-timing-options{display:flex;gap:14px;margin-bottom:12px}.edit-timing-options label{display:flex;align-items:center;gap:6px;font-size:11px}.edit-timing-options input{width:auto;margin:0}.edit-posting-times{display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:8px;margin-bottom:12px;padding:12px;border-radius:12px;background:#f8faf7}.edit-posting-times[hidden]{display:none!important}.edit-posting-times label{font-size:10px}.edit-posting-times input{margin-top:5px}.schedule-editor .btn{width:100%}
.ai-tone-picker{margin-top:14px;padding:15px;border:1px solid var(--line);border-radius:13px;background:#f8faf7}.ai-tone-picker[hidden]{display:none!important}.ai-tone-picker>strong{display:block;margin-bottom:10px;font-size:12px}.ai-tone-picker .choice-grid{grid-template-columns:repeat(3,1fr)}
.schedule-submit{display:inline-flex;align-items:center;justify-content:center;gap:9px}.schedule-submit[aria-busy="true"]{cursor:wait;opacity:.82}.schedule-submit-spinner{width:15px;height:15px;border:2px solid currentColor;border-right-color:transparent;border-radius:50%;animation:schedule-spin .7s linear infinite}.schedule-submit-status{margin:9px 0 0;color:var(--muted);font-size:11px}.schedule-submit-status[hidden],.schedule-submit-spinner[hidden]{display:none!important}@keyframes schedule-spin{to{transform:rotate(360deg)}}
...
@media(max-width:650px){
    .social-main{width:100%;min-width:0;overflow:hidden}
    .social-main .topline h1{font-size:24px}
    .template-panel,.schedule-form,.connection-panel,.schedule-list{min-width:0;padding:17px}
    .template-actions-row,.template-save-row{align-items:stretch;flex-direction:column}
    .platform-tabs{width:100%;max-width:100%;overflow-x:auto;overscroll-behavior-inline:contain;scrollbar-width:thin}
    .platform-tab{flex:0 0 auto;padding-inline:11px}
    .copy-template,.template-save-row .btn{width:100%}
    .template-workspace,.template-editor,.template-preview,.social-layout{width:100%;min-width:0}
    .template-preview{max-width:100%}
    .form-grid.two,.choice-grid,.timing-mode-grid,.ai-tone-picker .choice-grid{grid-template-columns:minmax(0,1fr)}
    .image-style-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
    .switch-row{align-items:flex-start}
    .switch-row input{flex:0 0 44px}
    .section-heading,.section-heading>div{align-items:flex-start}
    .section-heading{flex-direction:column}
    .section-heading>div>div{min-width:0}
    .status-pill{white-space:normal}
    .schedule-row,.upcoming-row{align-items:flex-start;flex-wrap:wrap}
    .schedule-row>div:first-child,.upcoming-row>div{flex-basis:calc(100% - 70px)}
    .schedule-row form{width:100%}
    .schedule-row form .text-button{width:100%;padding:9px;text-align:left}
    .schedule-editor{width:100%}
    .schedule-editor form{position:static;width:100%;max-width:100%;margin-top:9px;box-shadow:none}
    .edit-timing-options{align-items:flex-start;flex-direction:column}
    .posting-times,.edit-posting-times{grid-template-columns:minmax(0,1fr)}
    .prepared-caption{overflow-wrap:anywhere}
    .published-post-row{flex-direction:column}.published-post-heading{flex-direction:column}.published-post-edit .btn,.published-post-delete .btn{width:100%}
}
@media(max-width:380px){.image-style-grid{grid-template-columns:minmax(0,1fr)}.template-panel,.schedule-form,.connection-panel,.schedule-list{padding:14px}}
</style>
...
(() => {
    ...
    const copyModes = [...document.querySelectorAll('[data-copy-mode]')];
    const aiTones = document.querySelector('[data-ai-tones]');
    const scheduleForm = document.getElementById('schedule-form');
    const scheduleSubmit = scheduleForm?.querySelector('[data-schedule-submit]');
    const scheduleSubmitLabel = scheduleForm?.querySelector('[data-schedule-submit-label]');
    const scheduleSubmitSpinner = scheduleForm?.querySelector('[data-schedule-submit-spinner]');
    const scheduleSubmitStatus = scheduleForm?.querySelector('[data-schedule-submit-status]');

    scheduleForm?.addEventListener('submit', () => {
        if (!scheduleSubmit || scheduleSubmit.disabled) return;
        scheduleSubmit.disabled = true;
        scheduleSubmit.setAttribute('aria-busy', 'true');
        if (scheduleSubmitLabel) scheduleSubmitLabel.textContent = 'Creating schedule…';
        if (scheduleSubmitSpinner) scheduleSubmitSpinner.hidden = false;
        if (scheduleSubmitStatus) {
            scheduleSubmitStatus.textContent = 'Creating your schedule. Please wait…';
            scheduleSubmitStatus.hidden = false;
        }
    });

    document.getElementById('social-success-message')?.focus({preventScroll: false});

    function syncCopyMode() {
        const enabled = copyModes.find((mode) => mode.checked)?.value === 'ai';
        if (!aiTones) return;
        aiTones.hidden = !enabled;
        aiTones.querySelectorAll('input').forEach((input) => { input.required = enabled; });
    }

    copyModes.forEach((mode) => mode.addEventListener('change', syncCopyMode));
    syncCopyMode();
    ...
    document.querySelectorAll('.schedule-editor').forEach((editor) => {
        const modes = [...editor.querySelectorAll('[name="timing_mode"]')];
        const times = editor.querySelector('.edit-posting-times');
        modes.forEach((mode) => mode.addEventListener('change', () => {
            times.hidden = modes.find((item) => item.checked)?.value !== 'custom';
            times.querySelectorAll('input').forEach((input) => { input.required = !times.hidden; });
        }));
        times.querySelectorAll('input').forEach((input) => { input.required = !times.hidden; });
    });
