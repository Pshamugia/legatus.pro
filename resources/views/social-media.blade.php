                        ...
                        <div class="template-editor">
                            ...
                            <fieldset class="image-style-picker">
                                ...
                                <div class="image-style-grid">
                                    @foreach(['original' => 'Original', 'framed' => 'Classic frame', 'editorial' => 'Editorial', 'dark' => 'Dark', 'brand' => 'Legatus green'] as $style => $label)
                                        <label class="image-style-option">
                                            ...
                                            <span class="style-swatch {{ $style }}"><i></i></span><strong>{{ $label }}</strong>
                                        </label>
                                    @endforeach
                                </div>
                            </fieldset>
                            ...
                            <article class="social-preview {{ $provider }}">
                                ...
                                @if($provider === 'instagram')
                                    <div class="preview-image square image-style-{{ $configuration['image_style'] }}" data-preview-image="{{ $provider }}">
                                        @if($previewProduct['image'])<img src="{{ $previewProduct['image'] }}" alt="Preview of {{ $previewProduct['title'] }}">@else<span>Public product image</span>@endif
                                    </div>
                                ...
                                @else
                                    ...
                                    <div class="preview-image wide image-style-{{ $configuration['image_style'] }}" data-preview-image="{{ $provider }}">
                                        @if($previewProduct['image'])<img src="{{ $previewProduct['image'] }}" alt="Preview of {{ $previewProduct['title'] }}">@else<span>Link preview image</span>@endif
                                    </div>
...
.image-style-picker{margin:0 0 20px;padding:0;border:0}.image-style-picker legend{font-size:13px;font-weight:800}.image-style-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-top:11px}.image-style-option{cursor:pointer}.image-style-option input{position:absolute;opacity:0;pointer-events:none}.image-style-option strong{display:block;margin-top:5px;font-size:9px;text-align:center}.style-swatch{display:grid;place-items:center;aspect-ratio:1;border:2px solid transparent;border-radius:10px;background:#edf2ec}.style-swatch i{display:block;width:55%;height:70%;background:#fff;border:1px solid #ccd4ce}.image-style-option input:checked+.style-swatch{border-color:var(--green);box-shadow:0 0 0 3px rgba(30,91,69,.12)}.style-swatch.framed{background:#f6f2e8}.style-swatch.framed i{box-shadow:0 0 0 7px #fff,0 0 0 9px #1f4a3c}.style-swatch.editorial{background:#eef1eb}.style-swatch.editorial i{box-shadow:8px 8px 0 #beff49}.style-swatch.dark{background:#131d1a}.style-swatch.dark i{box-shadow:0 0 0 6px #222f2a}.style-swatch.brand{background:#def5e5}.style-swatch.brand i{box-shadow:0 0 0 7px #185d46}.preview-image:not(.image-style-original){padding:8%;box-sizing:border-box}.preview-image:not(.image-style-original) img{object-fit:contain;background:#fff}.preview-image.image-style-framed{background:#f6f2e8}.preview-image.image-style-framed img{box-shadow:0 0 0 12px #fff,0 0 0 15px #1f4a3c}.preview-image.image-style-editorial{background:#eef1eb}.preview-image.image-style-editorial img{box-shadow:16px 16px 0 #beff49}.preview-image.image-style-dark{background:#131d1a}.preview-image.image-style-dark img{box-shadow:0 0 0 12px #222f2a,0 0 0 15px #beff49}.preview-image.image-style-brand{background:#def5e5}.preview-image.image-style-brand img{box-shadow:0 0 0 12px #fff,0 0 0 15px #185d46}
